...
class DynamicTabsPage extends StatefulWidget {
  ...
  const DynamicTabsPage({Key key,@required this.sportsList}) : super(key: key);
    @override
    _DynamicTabsPageState createState() => _DynamicTabsPageState();
  }

  class _DynamicTabsPageState extends State<DynamicTabsPage> with AutomaticKeepAliveClientMixin<DynamicTabsPage>{
      ...
      return Scaffold(
        ...
        body: SafeArea(
          child: CustomTabView(
            ...
            tabBuilder: (context, index) => Tab(text: widget.sportsList[index].name),
            pageBuilder: (context, index) {
              print("ID: "+widget.sportsList[index].sportId.toString());
              return Container(child: PrayerProfiles(sport: widget.sportsList[index],));},
